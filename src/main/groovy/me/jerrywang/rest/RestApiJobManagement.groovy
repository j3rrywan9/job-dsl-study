...
class RestApiJobManagement extends MockJobManagement {
    ...
    String getPath(String name, boolean isView) {
        println "name passed to getPath: $name"
        if (name.startsWith('/')) {
    ...
    String fetchExistingXml(String name, boolean isView) {
        String testPath = getPath(name, isView) + '/config.xml'
        println "path in fetchExistingXml: $testPath"
